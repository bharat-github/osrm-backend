...
template <storage::Ownership Ownership> class CellStorageImpl
{
  ...
  private:
    ...
    template <typename WeightValueT, typename DurationValueT> class CellImpl
    {
      ...
      public:
        auto GetOutWeight(NodeID node) const { return GetOutRange(weights1, node); }

        auto GetInWeight(NodeID node) const { return GetInRange(weights1, node); }
        ...
        CellImpl(const CellData &data,
            ...
            : num_source_nodes{data.num_source_nodes},
              ...
              weights1{all_weights + data.value_offset},
  ...
  public:
    ...
    CellStorageImpl(const partition::MultiLevelPartition &partition, const GraphT &base_graph)
    {
        ...
        weights1.resize(value_offset + 1, INVALID_EDGE_WEIGHT);
    ...
    CellStorageImpl(Vector<EdgeWeight> weights_,
        ...
        : weights1(std::move(weights_)), durations(std::move(durations_)),
